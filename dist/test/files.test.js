...
var inst = src_1.$(fakeBot);
tape_1.default("setup", function (t) {
    t.end();
});
tape_1.default("$uperpower, handleExt helper passes through full filename", function (t) {
    var sample = 'a.json';
    var actual = inst.handleExt(sample);
    t.deepEqual(actual, sample);
    t.end();
});
tape_1.default("$uperpower, handleExt helper generates a file name when given just an extension", function (t) {
    var sample = '.json';
...
tape_1.default("$uperpower, handleExt helper generates a file name when given just an extension without a dot", function (t) {
    var sample = '.json';
...
tape_1.default("$uperpower, random name will generate distinct names", function (t) {
    var sample = inst.generateFileName();
    var sample2 = inst.generateFileName();
    t.notEqual(sample, sample2);
    t.end();
});
tape_1.default("$uperpower context: name ops are inverses of each other", function (t) {
    var key = 'my_key';
